/**
 * ルールのうち、fail/pass フィクスチャ 1 枚では踏めない分岐を直接固定するテスト。
 * 全ルール共通の表（fail で鳴る / pass で黙る / 重複ガード）は test/rule-table.ts と
 * そこに乗る test/rules.shard*.test.ts の担当。
 */
import * as fs from 'fs';
import * as os from 'os';
import * as path from 'path';
import { type Diagnostic, diagnose, diagnoseTemplate, engine } from './rule-table';
