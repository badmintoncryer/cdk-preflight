...
_pf_dbepwl_fix := "Give AdminUserPassword 8-100 characters, or set AuthType SECRET_ARN and pass a Secrets Manager secret ARN"
...
violation contains make_diag_full("pf-docdbelastic-admin-password-length", "ERROR", name,
	"Properties.AdminUserPassword",
	sprintf("AdminUserPassword is %d characters; CreateCluster accepts 8-100 (\"Invalid password set in the AdminUserPassword.\")", [count(pw)]),
	_pf_dbepwl_fix, _pf_dbepwl_url) if {
	some name in resources_of_type("AWS::DocDBElastic::Cluster")
	pw := resolve(name, "Properties.AdminUserPassword")
	is_string(pw)
