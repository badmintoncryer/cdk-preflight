...
# PutScheduledAction answers "Invalid schedule timezone: <value>" for anything
# outside the IANA time zone database — abbreviations (JST) and POSIX-style
# offsets (GMT+9) are both refused, while Etc/GMT+9 is accepted. The full name
# list is embedded because no cheap shape test separates the two: "JST" and
# "Etc/UTC" differ only by being in the database.
# ponytail: snapshot of tzdata 2026a via python zoneinfo, 598 names, backward links included.
# A zone added to IANA after this list would be reported wrongly;
# refresh the set when that happens.
_pf_aastz_zones := {
	...
	"Indian/Mayotte", "Indian/Reunion", "Iran", "Israel", "Jamaica", "Japan", "Kwajalein", "Libya",
	"MET", "MST", "MST7MDT", "Mexico/BajaNorte", "Mexico/BajaSur", "Mexico/General", "NZ",
	...
	"Pacific/Wallis", "Pacific/Yap", "Poland", "Portugal", "ROC", "ROK", "Singapore", "Turkey",
	"UCT", "US/Alaska", "US/Aleutian", "US/Arizona", "US/Central", "US/East-Indiana", "US/Eastern",
	"US/Hawaii", "US/Indiana-Starke", "US/Michigan", "US/Mountain", "US/Pacific", "US/Samoa",
	"UTC", "Universal", "W-SU", "WET", "Zulu",
}

violation contains make_diag_full("pf-appautoscaling-scheduled-action-timezone-iana", "ERROR", name,
	sprintf("Properties.ScheduledActions.%d.Timezone", [item.index]),
	sprintf("Timezone '%s' is not an IANA time zone name; PutScheduledAction fails with \"Invalid schedule timezone: %s\"", [tz, tz]),
	"Use an IANA name such as Asia/Tokyo, America/New_York or Etc/GMT+9",
	...
	sched := item.value
	is_object(sched)
	tz := object.get(sched, "Timezone", null)
	is_string(tz)
	not tz in _pf_aastz_zones
}
