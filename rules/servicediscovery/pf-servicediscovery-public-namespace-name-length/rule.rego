package cdk_preflight

import rego.v1

violation contains make_diag_full("pf-servicediscovery-public-namespace-name-length", "ERROR", name,
	"Properties.Name",
	sprintf("A public DNS namespace name is limited to 253 characters, but this one is %d", [count(n)]),
	"Shorten the namespace name to 253 characters or fewer",
	"https://docs.aws.amazon.com/cloud-map/latest/api/API_CreatePublicDnsNamespace.html") if {
	some name in resources_of_type("AWS::ServiceDiscovery::PublicDnsNamespace")
	n := _pf_sd_str(name, "Name")
	count(n) > 253
}
